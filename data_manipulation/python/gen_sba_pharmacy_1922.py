...
#
sbazip_pharmacy.columns = ['BorrZip','yq','NoLoan','GrossApproval']
sbazip_pharmacy['BorrZip'] = sbazip_pharmacy['BorrZip'].astype(str).str.zfill(5)
#
sbacty_pharmacy.columns = ['fips','yq','NoLoan','GrossApproval']
...
#
allpoi_cols = ['placekey', 'region','zip', 'latitude', 'longitude', 'location_name', 'street_address', 'city','naics_code', 'poi_cbg', 'nvisits', 'brands']       
drug19 = pl.scan_parquet('E:/pharmacy/advan/all_foottraffic_monthly_201901_202211_full.parquet').select(
    allpoi_cols + ['date_range_start','geometry_type']
).filter(
    (pl.col('date_range_start') == '2019-01-01') &
    (pl.col('geometry_type') != 'POINT') &
    (pl.col('naics_code') == '446110')
).collect().to_pandas()
drug19 = drug19[~drug19.region.str.contains('^AS$|^GU$|^MP$|^PR$|^VI$', na=False)]
drug19.placekey.duplicated().sum()
drug19.poi_cbg.isnull().sum()
drug19['tract'] = drug19.poi_cbg.str[:11]
drug19['fips'] = drug19.tract.str[:5]
#
ind19 = drug19[drug19.brands.isnull()]
#
...
indzip = indzip.merge(sodzip, left_on='zip', right_on='zip', how='left', indicator=True)
assert indzip[indzip._merge=='right_only'].shape[0] == 0 # Some borrowers located in zips without bank
...
indzip = indzip.merge(sbazip_pharmacy, left_on=['zip','yq'], right_on =['BorrZip','yq'], how='left', indicator=True)
indzip._merge.value_counts()
indzip.to_stata('C:/Users/elliotoh/Box/lodes_shared/pharmacy/data/advan/indzip_pharmacy_2019_2022.dta', write_index=False, version=118)
#
...
indcty._merge.value_counts()
indcty.to_stata('C:/Users/elliotoh/Box/lodes_shared/pharmacy/data/advan/indcty_pharmacy_2019_2022.dta', write_index=False, version=118)
...
sba1019 = sba_dispensed[(sba_dispensed.ApprovalDate.between('2010-01-01','2019-12-31'))].drop('_merge',axis=1)
sba_pharmacy1922 = sba1019[sba1019.NAICSCode=='446110.0']
sba_pharmacy1922.GrossApproval.describe()
